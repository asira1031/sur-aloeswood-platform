...
function normalizeRole(role?: string | null) {
  ...
  return value === "INVESTOR" ? "COPLANTER" : value;
}
...
export default function InvestorLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function verifyInvestorAccess() {
      setChecking(true);
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#06170f] p-6 text-white">
        <div className="rounded-[2rem] border border-white/10 bg-white/10 p-8 text-center shadow-2xl">
          <p className="text-xs font-black uppercase tracking-[0.3em] text-green-200">SUR Aloeswood Co-Planter</p>
          <h1 className="mt-4 text-3xl font-black">Checking access</h1>
          <p className="mt-3 text-sm text-white/70">Verifying your co-planter account before opening the workspace.</p>
        </div>
      </main>
    );
  }
