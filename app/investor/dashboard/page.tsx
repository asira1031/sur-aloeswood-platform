    ...
    <main className="min-h-screen bg-[#06170f] text-white">
      ...
      <section className="px-8 py-8 lg:px-14">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <QuickLink href="/investor/marketplace" title="Buy Seedlings" emoji="🌱" />
          <QuickLink href="/tree" title="My AG Trees" emoji="🌳" />
          <QuickLink href="/plantation" title="Plantation Timeline" emoji="🗓️" />
          <QuickLink href="/certificates" title="Certificates" emoji="📜" />
          <QuickLink href="/harvest" title="Harvest Timeline" emoji="🏆" />
        </div>

        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <Card title="Wallet Balance" value={money(wallet?.balance)} />
          <Card title="Recovery Fund" value={money(wallet?.recovery_balance)} />
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6">
            ...
            <div className="mt-6 grid gap-4">
              {trees.length === 0 ? (
                ...
                trees.map((tree) => (
                  <div key={tree.id} className="rounded-3xl border border-white/10 bg-black/20 p-5">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6">
              ...
              <div className="mt-4 grid gap-2 text-sm">
                {[
                  ...
                  <div key={item} className="rounded-2xl bg-black/20 px-4 py-3 font-bold text-white/80">
                    {item}
                  </div>
                ))}
              </div>
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-xl transition hover:bg-white/[0.1]"
    >
      <p className="text-3xl">{emoji}</p>
      <p className="mt-3 font-black text-green-100">{title}</p>
    </Link>
  );
}
