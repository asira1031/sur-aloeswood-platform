...
export default function AdminDashboardPage() {
  ...
  async function loadDashboard() {
    ...
    setNotice(errors.length ? "Some information could not be loaded. Refresh once; if it continues, open System Safety below." : ""); setLoading(false);
  }
  ...
  return <main className="min-h-screen bg-[#f4f1e7] px-4 py-5 text-[#10271f] sm:px-6 lg:px-8"><div className="mx-auto w-full max-w-[1380px]">
    ...
    <section className="mt-5 rounded-[2rem] border border-[#ded8c7] bg-white p-5 shadow-sm sm:p-7"><p className="text-xs font-black uppercase tracking-[.18em] text-[#167553]">System safety</p><h2 className="mt-2 text-2xl font-black">Check first. Repair safely.</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#687970]">Use these only when the app reports a problem. Normal daily work stays in Accounts, Tree, Tasks, and Support.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Link href="/health" className="rounded-2xl border border-[#dbe5df] bg-[#f7faf8] p-5"><b className="block">Check system health</b><span className="mt-2 block text-sm text-[#687970]">See whether the app can reach its important services.</span></Link><Link href="/admin/recovery" className="rounded-2xl border border-[#e5d5a9] bg-[#fff9e8] p-5"><b className="block">Open safe recovery</b><span className="mt-2 block text-sm text-[#68501b]">Review a stuck workflow before taking a repair action.</span></Link></div><details className="mt-4 rounded-2xl border border-[#dbe5df] bg-white p-4"><summary className="cursor-pointer text-sm font-black text-[#53665d]">Developer assistance</summary><p className="mt-3 text-sm leading-6 text-[#718078]">Open this only when System Health or Safe Recovery says developer help is needed.</p><Link href="/admin/guardian" className="mt-3 inline-flex rounded-xl border border-[#cdd9d2] px-4 py-2.5 text-sm font-black text-[#073d2e]">Open technical assistant</Link></details></section>
  </div></main>;
}
